...
. "$env:SCOOP_HOME\test\..\lib\core.ps1"
. "$env:SCOOP_HOME\test\..\lib\manifest.ps1"

describe "manifest-validation" {
    beforeall {
        $working_dir = setup_working "manifest"
        $schema = "$env:SCOOP_HOME\test\..\schema.json"
        Add-Type -Path "$env:SCOOP_HOME\test\..\supporting\validator\bin\Newtonsoft.Json.dll"
        Add-Type -Path "$env:SCOOP_HOME\test\..\supporting\validator\bin\Newtonsoft.Json.Schema.dll"
        Add-Type -Path "$env:SCOOP_HOME\test\..\supporting\validator\bin\Scoop.Validator.dll"
    }
